...
from util.visualizer import save_LAG_imgs
import matplotlib
matplotlib.use('Agg')
...
if __name__ == '__main__':
    ...
    for k in range(begin, end + 1, 1):
        ...
        model_name = 'LAG_%s_%d_%d%s' % (model, k,K, suffix)
        opt.dataroot = '../datasets/LAG_database_part_1/'
        opt.name = model_name
        opt.dataset_mode = 'LAG'
        opt.model = model
        ...
        for i, data in enumerate(dataset):
            ...
            save_LAG_imgs(visuals, img_path, model_name, opt.epoch)
