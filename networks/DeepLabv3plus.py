...
# mobilenet_v2
from networks.MobileNet_v2 import MobileNet_v2_os_32, MobileNet_v2_os_8
import pdb
...
class DeepLabv_v3_plus_mv2_os_32(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=[160, 100])
...
class DeepLabv_v3_plus_mv2_os_8(nn.Module):
    ...
    def forward(self, x):
        ...
        # x : 1/8 64 x 64       x = bs,3,128,128
        x, low_level_features = self.mobilenet_features(x)      # x = bs,320,16,16      low_level_features = bs,24,32,32

        x_aspp = self.aspp(x)       # bs,1024,16,16
        x_ = self.global_avg_pool(x)        # bs,256,1,1
        x_ = F.upsample(x_, size=x.size()[2:], mode='bilinear', align_corners=True)     # bs,256,16,16
        x = torch.cat((x_aspp, x_), dim=1)      # bs,1280,16,16

        x = self.conv(x)    # bs,256,16,16
        x = self.bn(x)      # bs,256,16,16
        x = F.upsample(x, scale_factor=2, mode='bilinear', align_corners=True)      # bs,256,32,32

        # 1/4 128 x 128
        low_level_features = self.conv2(low_level_features)     # bs,48,32,32
        low_level_features = self.bn2(low_level_features)       # bs,48,32,32

        x = torch.cat((x, low_level_features), dim=1)       # bs,304,32,32

        x = self.last_conv(x)       # bs,3,32,32
        x = F.upsample(x, scale_factor=4, mode='bilinear', align_corners=True)      # bs,3,128,128

        return x

if __name__ == "__main__":
    model = DeepLabv_v3_plus_mv2_os_8()
