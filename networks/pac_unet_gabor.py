...
import inspect
from detail_preprocess.Gabor import get_gabor_feature,GaborConv2d
from detail_preprocess.Wavelet import get_wavelet_feature

class PacUnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, attention=False, guide_channels=12,gpu_ids=[]):
        super(PacUnetGenerator, self).__init__()

        # self.gpu_ids = gpu_ids

        # self.gabor_conv2d_1 = GaborConv2d(channel_in=input_nc, channel_out=4, kernel_size=3, padding=1)
        # self.gabor_conv2d_2 = GaborConv2d(channel_in=input_nc, channel_out=4, kernel_size=7, padding=3)
        # self.gabor_conv2d_3 = GaborConv2d(channel_in=input_nc, channel_out=4, kernel_size=11, padding=5)
        self.gabor_conv2d = GaborConv2d(channel_in=input_nc, channel_out=3, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        # prepare guide
        # g1 = self.gabor_conv2d_1(x)
        # g2 = self.gabor_conv2d_2(x)
        # g3 = self.gabor_conv2d_3(x)
        # g = torch.cat((g1,g2,g3),1)
        g = self.gabor_conv2d(x)
        # g = self.get_texture_features(x)
        guide = self.branch_g(g)
        # guide = x

        # bs,3,128,128
        # down
        x1 = self.downconv1(x, guide[:,0*self.k_ch:1*self.k_ch])                                  # bs,64,64,64
        x2 = self.downnorm2(self.downconv2(self.downrelu(x1), F.interpolate(guide[:,1*self.k_ch:2*self.k_ch], scale_factor=math.pow(0.5,1), mode='nearest')))   # bs,128,32,32
        x3 = self.downnorm3(self.downconv3(self.downrelu(x2), F.interpolate(guide[:,2*self.k_ch:3*self.k_ch], scale_factor=math.pow(0.5,2), mode='nearest')))   # bs,256,16,16
        x4 = self.downnorm4(self.downconv4(self.downrelu(x3), F.interpolate(guide[:,3*self.k_ch:4*self.k_ch], scale_factor=math.pow(0.5,3), mode='nearest')))   # bs,512,8,8
        x5 = self.downnorm5(self.downconv5(self.downrelu(x4), F.interpolate(guide[:,4*self.k_ch:5*self.k_ch], scale_factor=math.pow(0.5,4), mode='nearest')))    # bs,512,4,4
        x6 = self.downnorm6(self.downconv6(self.downrelu(x5), F.interpolate(guide[:,5*self.k_ch:6*self.k_ch], scale_factor=math.pow(0.5,5), mode='nearest')))    # bs,512,2,2
        # innermost
        x7 = self.downconv7(self.downrelu(x6), F.interpolate(guide[:,6*self.k_ch:7*self.k_ch], scale_factor=math.pow(0.5,6), mode='nearest'))                    # bs,512,1,1
        x7 = self.upnorm7(self.upconv7(self.uprelu(x7), F.interpolate(guide[:,6*self.k_ch:7*self.k_ch], scale_factor=math.pow(0.5,6), mode='nearest')))          # bs,512,2,2
        x7 = torch.cat([x6, x7], 1)                                       # bs,1024,2,2
        # up
        x6_ = self.upnorm6(self.upconv6(self.uprelu(x7), F.interpolate(guide[:,5*self.k_ch:6*self.k_ch], scale_factor=math.pow(0.5,5), mode='nearest')))         # bs,512,4,4
        x6_ = torch.cat([x5, x6_], 1)                                     # bs,1024,4,4
        x5_ = self.upnorm5(self.upconv5(self.uprelu(x6_), F.interpolate(guide[:,4*self.k_ch:5*self.k_ch], scale_factor=math.pow(0.5,4), mode='nearest')))        # bs,512,8,8
        x5_ = torch.cat([x4, x5_], 1)                                     # bs,1024,8,8
        x4_ = self.upnorm4(self.upconv4(self.uprelu(x5_), F.interpolate(guide[:,3*self.k_ch:4*self.k_ch], scale_factor=math.pow(0.5,3), mode='nearest')))        # bs,256,16,16
        x4_ = torch.cat([x3, x4_], 1)                                     # bs,512,16,16
        x3_ = self.upnorm3(self.upconv3(self.uprelu(x4_), F.interpolate(guide[:,2*self.k_ch:3*self.k_ch], scale_factor=math.pow(0.5,2), mode='nearest')))       # bs,128,32,32
        x3_ = torch.cat([x2, x3_], 1)                                     # bs,256,32,32
        x2_ = self.upnorm2(self.upconv2(self.uprelu(x3_), F.interpolate(guide[:,1*self.k_ch:2*self.k_ch], scale_factor=math.pow(0.5,1), mode='nearest')))       # bs,64,64,64
        x2_ = torch.cat([x1, x2_], 1)                                     # bs,128,64,64
        x1_ = self.tanh(self.upconv1(self.uprelu(x2_), guide[:,0*self.k_ch:1*self.k_ch]))          # bs,3,128,128
...
if __name__ == '__main__':
    ...
    use_dropout = True
    pacunet = PacUnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout).to(device)
    gpu_tracker.track()
